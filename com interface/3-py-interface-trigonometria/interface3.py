...
# Importa as classes Image e ImageTK da biblioteca para manipulação de imagens
import os 
# Importa a biblioteca os para operações com o sistema de arquivos
import sys 
# Importa a biblioteca sys para manipulação de variáveis e funções do sistema
...
def resource_path(relative_path):
    ...
    try:
        # PyInstaller crua um diretório temporário e armazena o caminho em _MEIPASS
...
def calcular():
    """
    Realiza o cálculo dos valores trigonométricos (seno, cosseno e tangente) do ângulo fornecido e atualiza os labels com os resultados.
    """
    try:
...
def validar_entrada(texto):
    """
    Valida a entrada do usuário permitindo apenas números que o valor esteja entre 0 e 90.
    """
    if texto.isdigit() or texto == "":  # Permite apenas números ou campo vazio
        ...
        return 0 <= valor <=90 
    # Retorna True se o valor estiver entre 0 e 90, caso contrário False
...
try:
    ...
    foto = ImageTk.PhotoImage(imagem) # Converte a imagem para um formato compatível com o Tkinter
    ...
    # Caso a imagem não seja encontrada, exibi uma mensagem de texto no lugar da imagem
...
label_angulo = tk.Label(frame_entrada, text="Ângulo (0 á 90):", font=('Arial', 14), bg="#f0f0f0") # Label para o campo de entrada
...
entrada_angulo = tk.Entry(frame_entrada, width=3, justify='center', font=('Arial', 16), bd=0, highlightthickness=0, relief='flat', bg="#f0f0f0", fg='red', validate="key", validatecommand=(validacao, '%P')) # Criar o campo de entrada do ângulo
entrada_angulo.pack() # Posiciona o campo de entrada

# Linha abaixo do campo de entrada
linha = tk.Frame(frame_entrada, bg="black", height=1, width=entrada_angulo.winfo_reqwidth()) # Criar uma linha decorativa abaixo do campo de entrada
...
frame_botoes.pack(pady=20) # Posiciona o frame na janela comum espaçamento vertical

botao_calcular = tk.Button(frame_botoes, text="Calcular", command=calcular, font=("Arial", 12), bg='#d9d9d9', relief='flat', bd=0, highlightthickness=0) # Botão para calcular os valores trigonométricos~
botao_calcular.pack(side=tk.LEFT, padx=10) # Posiciona o botão à esquerda com um espaçamento horizontal

botao_limpar = tk.Button(frame_botoes, text="Limpar", command=limpar, font=("Arial", 12), bg='#d9d9d9', relief='flat', bd=0, highlightthickness=0) # Botão para limpar os valores trigonométricos
botao_limpar.pack(side=tk.RIGHT, padx=10) # Posiciona o botão à esquerda com um espaçamento horizontal
...
# Label e resultado para o tangente
label_tangente = tk.Label(frame_resultados, text="Tangente:", font=('Arial', 12), bg="#f0f0f0")
label_tangente.grid(row=2, column=0, padx=10, pady=5, sticky='e') # Label para o tangente, alinhado à direita
resultado_tangente = tk.Label(frame_resultados, text="", font=('Arial', 12, 'bold'), fg='red', bg="#f0f0f0")
resultado_tangente.grid(row=2, column=1, padx=10, pady=5, sticky='w') # Label que exibe o resultado do tangente, alinhado à esquerda
